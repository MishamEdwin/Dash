...
const OverAllReview = ({ selectedDate }) => {  
  ...
  const [lastFiveYearsRawData, setLastFiveYearsRawData] = useState({}); 
  // New State  
  const [allData, setAllData] = useState({});  
  const [cordysTatData, setCordysTatData] = useState([]); // Added specific state for Cordys TAT 
...
[];  
        ...
        const cordysTatJson = getFile('CORDYS_TAT_REPORT') || []; // Load new TAT Data 
        ...
        // Process Cordys TAT Data 
        const processedCordys = []; 
        if (Array.isArray(cordysTatJson)) { 
            cordysTatJson.forEach(group => { 
                if (group.Month && Array.isArray(group.Data)) { 
                    group.Data.forEach((row, idx) => { 
                         processedCordys.push({ 
                             month: group.Month, 
                             rowSpan: idx === 0 ? group.Data.length : 0, 
                             lob: row.LOB, 
                             sameDay: row["Same Day"]?.["No of RFQ's"] || 0, 
                             sameDayPct: row["Same Day"]?.["%"] || "0%", 
                             nextDay: row["Next day"]?.["No of RFQ's"] || 0, 
                             nextDayPct: row["Next day"]?.["%"] || "0%", 
                             beyond: row["Beyond that"]?.["No of RFQ's"] || 0, 
                             beyondPct: row["Beyond that"]?.["%"] || "0%", 
                             total: row["Grand Total"] || 0, 
                             isTotal: row.LOB && row.LOB.trim().includes('Total'), 
                             type: 'data' 
                         }); 
                    }); 
                } else if (group["Overall Total"]) { 
                     const totalData = group["Overall Total"]; 
                     processedCordys.push({ 
                         month: "Overall Total", 
                         lob: "", 
                         sameDay: totalData["Same Day"]?.["No of RFQ's"] || 0, 
                         sameDayPct: totalData["Same Day"]?.["%"] || "0%", 
                         nextDay: totalData["Next day"]?.["No of RFQ's"] || 0, 
                         nextDayPct: totalData["Next day"]?.["%"] || "0%", 
                         beyond: totalData["Beyond that"]?.["No of RFQ's"] || 0, 
                         beyondPct: totalData["Beyond that"]?.["%"] || "0%", 
                         total: totalData["Grand Total"] || 0, 
                         isTotal: true, 
                         type: 'overall' 
                     }); 
                } 
        ...
        setCordysTatData(processedCordys); 
...
liability: [], sme: [] };  
  ...
  const popupData = allData?.popupDataMap?.[selectedPopupYTD] || [];  
          ...
          <div className="or-table-block">  
            ...
            <table className="or-table-block-table">  
              <thead>  
                ...
                <tr>  
                  <th className="or-table-block-th">No</th><th className="or-table-block-th">%</th> 
                  <th className="or-table-block-th">No</th><th className="or-table-block-th">%</th> 
                  <th className="or-table-block-th">No</th><th className="or-table-block-th">%</th>  
                </tr>  
              </thead>  
              <tbody>  
                {cordysTatData.map((row, idx) => (  
                  <tr key={idx}  
                    className={row.isTotal ? "or-table-tr-total" : (idx % 2 === 0 ? "or-table-block-tr-even" : "or-table-block-tr-odd")} 
                    style={(row.isTotal) ? { backgroundColor: '#fbcfe8', fontWeight: 'bold' } : {}} 
                  >  
                    {row.type === 'overall' ? ( 
                       <td className="or-table-block-td" colSpan={2} style={{textAlign: 'center', fontWeight: 'bold'}}>Overall Total</td> 
                    ) : ( 
                       row.rowSpan > 0 && ( 
                        <td rowSpan={row.rowSpan} className="or-table-block-td" style={{verticalAlign: 'middle', backgroundColor: '#fff'}}>{row.month}</td> 
                       ) 
                    )} 
...
right: '15px', background: 'none', border: 'none', fontSize: '20px', cursor: 'pointer', color: '#666' 
            ...
            <h2 style={{ marginBottom: '20px', textAlign: 'center' }}>LOB & SEGMENT WISE REPORT LAST 5 
YEARS COMPARISON</h2>  
              
            {/* TABLE 1: LOB & Segment Wise Report (YTD Data) */}  
            <div style={{ marginBottom: '30px' }}>   
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', 
backgroundColor: '#ff6600', padding: '8px', marginBottom: '0' }}>  
                 <h3 style={{ color: 'white', margin: '0', textAlign: 'center', flex: 1, fontSize: '14px' }}>LOB & 
Segment wise Report - Current Financial Year</h3>  
                 <select   
                 ...
                 >   
                    {popupYTDPeriods.length > 0 ? popupYTDPeriods.map(p => <option key={p} 
value={p}>{p}</option>) : <option>No YTD Data</option>}   
                 </select>   
              </div>  
...
backgroundColor: '#ff6600', padding: '8px', marginBottom: '0' }}>  
                 <h3 style={{ color: 'white', margin: '0', textAlign: 'center', flex: 1, fontSize: '14px' }}>LOB & 
Segment wise Report - Previous Financial Years</h3>  
                 <select   
                    value={selectedPopupCompare}   
                    onChange={(e) => setSelectedPopupCompare(e.target.value)}   
                    style={{ padding: '2px 8px', borderRadius: '4px', fontSize: '12px', color: 'black' }}  
                 >   
                    {popupComparePeriods.length > 0 ? popupComparePeriods.map(p => <option key={p} 
value={p}>{p}</option>) : <option>No Data</option>}   
                 </select>   
              </div>  
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '12px' }}>   
                <thead>   
...
'#00cccc' }}>LIABILITY</th>   
                ...
                <tbody>   
                  {popupTable2Data.map((row, i) => (   
                    <tr key={i} style={row.Segment === 'TOTAL' ? { backgroundColor: '#fbcfe8', fontWeight: 
'bold' } : { backgroundColor: i % 2 === 0 ? '#f9f9f9' : 'white' }}>   
                      <td style={{ border: '1px solid #ccc', padding: '4px', fontWeight: 'bold' 
}}>{row.Segment}</td>   
                      {['FIRE', 'ENGINEERING', 'MISCELLANEOUS', 'Marine', 'Liability', 'Overall'].map(colKey => {  
                          ...
                          return (  
                             <Fragment key={colKey}>  
                                <td style={{ border: '1px solid #ccc', padding: '8px', textAlign: 'right' }}>{cell.NOP ? 
cell.NOP.toLocaleString() : 0}</td>  
                                <td style={{ border: '1px solid #ccc', padding: '8px', textAlign: 'right' }}>{cell.GWP ? 
Math.round(cell.GWP) : 0}</td>  
                                <td style={{ border: '1px solid #ccc', padding: '8px', textAlign: 'right' }}>{cell['GIC:GEP'] 
? Math.round(cell['GIC:GEP']*100) : 0}%</td>  
                             </Fragment>  
                          )  
                      })}  
                    </tr>   
